...
import { createDocument, getDocumentsByUser, Document } from '../database/db';
import { INDIAN_LEGAL_DB, LegalTopic } from './legalDb';
 ...
 */
export function splitTextIntoChunks(text: string, chunkSize = 800, chunkOverlap = 150): string[] {
  if (!text) return [];
  const chunks: string[] = [];
  let index = 0;
  ...
  while (index < text.length) {
    ...
    if (index >= text.length - chunkOverlap) break;
  }
  return chunks;
}
 ...
 */
export async function extractTextFromFile(buffer: Buffer, fileName: string, fileType: string): Promise<string> {
...
export async function searchRAG(
  query: string,
  userId: string,
  options: { maxResults?: number; chatId?: string; documentName?: string } = {}
): Promise<{ content: string; source: string; score: number; isUserDoc: boolean }[]> {
  ...
  const results: { content: string; source: string; score: number; isUserDoc: boolean }[] = [];
  ...
  if (!useLocalRag && openai && pineconeIndex) {
    try {
      ...
      const pineconeFilter: any = { chat_id: options.chatId || 'no-chat-matching' };
        ...
        for (const match of queryResponse.matches) {
          ...
          if (match.metadata && match.score && match.score >= threshold) {
            results.push({
              ...
              source: (match.metadata.document_name as string) || 'User Document',
              score: match.score,
              isUserDoc: true
            });
          }
  ...
  const staticMatches: { content: string; source: string; score: number; isUserDoc: boolean }[] = [];
  for (const topic of INDIAN_LEGAL_DB) {
    let score = 0;
