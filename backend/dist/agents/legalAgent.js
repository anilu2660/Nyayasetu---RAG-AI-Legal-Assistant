...
exports.isLegalQuery = isLegalQuery;
exports.streamLegalAssistantResponse = streamLegalAssistantResponse;
const openai_1 = require("openai");
const pipeline_1 = require("../rag/pipeline");
 ...
 */
async function* streamLegalAssistantResponse(query, chatId, userId, documentName) {
    ...
    const history = await (0, db_1.getMessagesByChatId)(chatId);
    const formattedHistory = history.map(h => ({
        role: h.role,
        content: h.content
    }));
    // 3. Search RAG for context
    const ragResults = await (0, pipeline_1.searchRAG)(query, userId, { chatId, documentName });
